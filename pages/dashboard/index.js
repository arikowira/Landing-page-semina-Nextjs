...
export default function Dashboard({ dataOrders, dataProfile }) {
  return (
    <>
      <Head>
...
export async function getServerSideProps(context) {
  const reqOrders = await getData(
    `/api/v1/orders`,
    {},
    context.req.cookies.token
  );
  const orders = reqOrders.data;

  const reqProfile = await getData(
    `/api/v1/profile`,
    {},
    context.req.cookies.token
  );
  const profile = reqProfile.data;
  if (profile?.response?.data?.msg === 'jwt expired') {
    return {
      redirect: {
        destination: '/signin',
        permanent: false,
    ...
    return {
      props: { dataOrders: orders, dataProfile: profile },
    };
  }
}
